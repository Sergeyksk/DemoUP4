...
Процедура ЗаполнитьСписокПользователей()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.ЦеныТоваров.СрезПоследних(&ДатаЦены, ) КАК ЦеныТоваровСрезПоследних
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Товары КАК Товары
		|		ПО ЦеныТоваровСрезПоследних.Товар = Товары.Ссылка";
	
	Запрос.УстановитьПараметр("ДатаЦены", ТекущаяДатаСеанса());
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Сообщить(ВыборкаДетальныеЗаписи.Ссылка.Наименование);
	КонецЦикла;
